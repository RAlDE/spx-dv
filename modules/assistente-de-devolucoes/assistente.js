(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .driver{margin-top:9px;color:#fff;font-size:19px;line-height:1.25}#${PANEL_ID} .driver .driver-code{color:#dbeafe}#${PANEL_ID} .carrier{margin-top:4px;color:#fff;font-size:19px;line-height:1.25}#${PANEL_ID} time{color:#fff;font-size:19px;font-weight:700;white-space:nowrap}#${PANEL_ID} img{width:58px;height:58px;margin-top:8px;border-radius:7px;object-fit:cover}
  ...
  function getDriverCarrier(attempt) {
    const candidates = [
      attempt?.driver_company_name,
      attempt?.driver_company,
      attempt?.company_name,
      attempt?.carrier_name,
      attempt?.service_provider_name,
      attempt?.fleet_provider_name,
      attempt?.fleet_name,
      attempt?.agency_name,
      attempt?.contractor_name,
      attempt?.transportation_company_name,
      attempt?.transportation_company,
      attempt?.driver?.company_name,
      attempt?.driver?.carrier_name,
      attempt?.driver?.fleet_name,
      attempt?.driver_info?.company_name,
      attempt?.driver_info?.carrier_name,
      attempt?.driver_info?.fleet_name
    ];
    return String(candidates.find(value => value !== undefined && value !== null && String(value).trim()) || 'Não informada');
  }
